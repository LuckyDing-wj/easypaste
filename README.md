...
一个简单的网络剪切板，部署在 Cloudflare Pages 上。打开网页，输入密钥，就能粘贴和读取文本内容。

## 它能做什么

- 在任何设备上打开网页，输入密钥就能用
- 粘贴一段文字，点保存，内容就存到云端了
- 换个设备打开同一个网页，输入密钥，就能看到之前保存的内容
...
│   ├── _redirects             # 告诉 Cloudflare 把所有请求都指向 index.html
...
8. **记下这行右边的 ID**（一串字母和数字，类似 `a1b2c3d4e5f6...`），等下要用
...
1. 打开你的网站地址（在 Pages 项目的 **Deployments** 页面可以看到，格式类似 `https://xxx.pages.dev`）
2. 输入你设置的密钥（就是第五步填的那个 UUID）
3. 点击 **进入**
4. 现在你可以：
   - 在文本框里粘贴内容
...
**Q：打开网页显示 404？**
A：检查 `_redirects` 文件是否在 `public` 文件夹里。Cloudflare Pages 只会托管 `public` 文件夹里的内容。

**Q：输入密钥后显示"密钥无效"？**
A：检查环境变量 `CLIP_SECRET` 是否设置正确，是否有多余的空格。
